...
function showResult() {
    ...
    if (passed) {
        resultTitle.textContent = '你就是提瓦特的记录者';
        resultDesc.textContent = '答对了 ' + correctCount + '/5 题，你对提瓦特的历史了如指掌！';
        resultTitle.className = 'result-title pass';
    } else {
        resultTitle.textContent = '再试一次吧';
        resultDesc.textContent = '答对了 ' + correctCount + '/5 题，多看看原神的剧情故事吧！';
        resultTitle.className = 'result-title fail';
    }
}

function retryQuiz() {
    window.location.href = '/quiz/extreme';
}
